...
### Sort issue: divergent transitions after warmup
...
We have some divergent transitions (160 to be precise) so let's thin by 3 and set adapt_delta to 0.95.



```{r Sh_all-drop9-max-v2, message=FALSE, warning=FALSE, cache=TRUE}
### Drop ph
max8th_drop_ph_v2 <-
  brm(data = snailDF.na, 
      family = bernoulli(link = logit),
      ...
      thin = 3, # Thin chain by 3
      inits  = 0, seed = 666,
      control = list(adapt_delta = 0.95)) # Set adapt_delta to 0.95
# Save object
saveRDS(max8th_drop_ph_v2, "../Data/ModelOutputs/InfectionPrevalence/max8th_drop_ph_v2.rds")
```


We still have divergent transitions but only 8 instead of 160 so increasing adapt_delta definitely helped. Let's rerun the model, further reducing adapt_delta to 0.99 (max is 1)  and keep thinning = 3.

```{r Sh_all-drop9-max-v3, message=FALSE, warning=FALSE, cache=TRUE}
### Drop ph
max8th_drop_ph_v3 <-
  brm(data = snailDF.na, 
      family = bernoulli(link = logit),
...
# Save object
saveRDS(max8th_drop_ph_v3, "../Data/ModelOutputs/InfectionPrevalence/max8th_drop_ph_v3.rds")
```
...
1. Drop depth as explanatory variable because temperature and depth are highly correlated.   
2. Drop temperature as explanatory variable because temperature and depth are highly correlated.  
3. Drop depth as explanatory variable *AND* make survey random factor because we are not only interested in these particular surveys?   
4. Drop temperature as explanatory variable *AND* make survey random factor because we are not only interested in these particular surveys?   

```{r Sh_all-drop9-max-v4, message=FALSE, warning=FALSE, cache=TRUE}
### Drop depth
max9th_drop_depth <-
  brm(data = snailDF.na, 
      family = bernoulli(link = logit),
