...
temp and ph are clearly collinear. From hereon, only temp will be used.
...
# Zero inflated poisson with a single zeroinflation parameter applying to all observations
require(glmmTMB)
glmm.ZIpoisson <- glmmTMB(BgBn_number ~ wlevel + type + survey +
                         arm::rescale(temp) + arm::rescale(cond) + 
                          (1|shehia/site), data=siteDF,
                         ziformula=~1, # one zero inflation parameter applying to all observations
                         family=poisson)

llikAIC(glmm.poisson) # 6716.111
glmm.ZIpoisson # 4548.511

plot(simulateResiduals(glmm.poisson,  n = 1000), rank = TRUE)
plot(simulateResiduals(glmm.ZIpoisson,  n = 1000), rank = TRUE)


### Although a zero inflated poisson model hugely improves model fit as evident from AIC values (6716 vs 4549), and QQplots, the fit is not ideal and the model does not converge. Lets try the negative binomial distribution


# Negative binomial
glmm.nb <- glmer.nb(BgBn_number ~ wlevel + type + survey +
                         arm::rescale(temp) + arm::rescale(cond) + 
                          (1|shehia/site), data=siteDF)
# test for zero-inflation
testZeroInflation(simulateResiduals(glmm.nb,  n = 1000)) # No evidence for zero inflation 

# Zero inflated negative binomial with a single zeroinflation parameter applying to all observations
glmm.ZInb <- glmmTMB(BgBn_number ~ wlevel + type + survey +
                         arm::rescale(temp) + arm::rescale(cond) + 
                          (1|shehia/site), data=siteDF,
                         ziformula = ~1,
                         family = nbinom2)

llikAIC(glmm.nb) # 1742.5418
glmm.ZInb # 1740.3436
...
glmm.nb.min.1 <- glmer.nb(BgBn_number ~ wlevel + type + survey +
                          arm::rescale(temp) +
                          (1|shehia/site), data=siteDF)
...
summary(glmm.nb.min.1) # 1743.6
summary(glmm.ZInb.min.1) # 1739.2

plot(simulateResiduals(glmm.nb.min.1,  n = 1000), rank = TRUE)
plot(simulateResiduals(glmm.ZInb.min.1,  n = 1000), rank = TRUE)
...
\n
I think model 3 shows a very nice n snug fit although the others aren't too bad either. Lets look at whether they converged:



```{r glmer-BgBn_number-nb-min-models-summary, message=FALSE, warning=FALSE, cache=TRUE}
# summary
anova(glmm.nb.min.1, glmm.nb.min.2, glmm.nb.min.3, glmm.nb.min.4)
```
Right so model 1-3 reached convergence (whilst 4 did not) in the end and model three has the best fit - as good if not better than the original maximal model - and the best (i.e.) lowest ACI among the models that reached convergence. 

### Final model
Test whether there is evidence for spatial or temporal autocorrelation. 
```{r glmm.nb.min.3-autocorrelation, cache=TRUE, message=FALSE}
## Temporal
mydata <- (siteDF[complete.cases(siteDF[, c("BgBn_number", "wlevel", "type", "survey",
...
glmm.ZINB.min.3 <- glmmTMB(BgBn_number ~ wlevel + type + survey +
...
## Spatial
require(DHARMa)
E <- simulateResiduals(glmm.nb.min.1,  n = 1000)

E2 <- resid(glmm.nb.min.1)
require(gstat)
require(sp)
mydata <- (siteDF[complete.cases(siteDF[, c("BgBn_number", "wlevel", "type", "survey",
                                           "temp", "shehia", "site", "lat_cor", "lon_cor", "survey_date")]),])
...
testSpatialAutocorrelation(glmm.ZInb.min.1, x = mydata$lat_cor, y = mydata$lon_cor)
...
coordinates(siteDF_res) <- c("siteDF_coord.lon_cor", "siteDF_coord.lat_cor")
