...
temp and ph are clearly collinear 
...
```{r glmm.nb.min.3-autocorrelation, cache=TRUE}
E <- resid(glmm.nb.min.3)
require(gstat)
require(sp)
mydata <- (siteDF[complete.cases(siteDF[, c("BgBn_number", "wlevel", "type", "survey",
                                           "temp", "shehia", "site", "lat_cor", "lon_cor", "survey_date")]),])
mydata <- data.frame(E, mydata$lat_cor, mydata$lon_cor)
coordinates(mydata) <- c("mydata.lat_cor","mydata.lon_cor")
...
vgm1 <- gstat::variogram(E∼x+y, mydata)
plot(vgm1)
```
